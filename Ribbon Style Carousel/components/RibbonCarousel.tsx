...
import { CAROUSEL_CONTENT } from '../data/content';
...
const RibbonCarousel: React.FC = () => {
  ...
  const [activeFunbox, setActiveFunbox] = useState<FunboxSession | null>(null);
  ...
  const currentScale = useMemo(() => {
    ...
    if (isPortrait) return Math.max(scaleH * 0.95, scaleW * 1.1);
    return Math.min(scaleW * 1.1, scaleH);
  ...
  useAnimationFrame((_, delta) => {
    if (selectedItem || activeFunbox || isMobile) return;
  ...
  const handleLaunchFunbox = (session: FunboxSession) => setActiveFunbox(session);
  ...
  if (isMobile) {
    return (
      <div className="w-full h-full overflow-y-auto no-scrollbar pt-20 pb-40 px-6">
        ...
        <MediaModal item={selectedItem} onClose={handleClose} isMobile={true} onLaunchFunbox={handleLaunchFunbox} />
        <FunboxRuntime session={activeFunbox} onClose={() => setActiveFunbox(null)} />
      </div>
    );
  }
  ...
  return (
    <div className="w-full h-full flex items-center justify-center overflow-hidden bg-[#020617]">
        ...
        <div className="absolute top-0 left-0 w-full h-full pointer-events-auto">
          {CAROUSEL_CONTENT.map((item, index) => (
            <CarouselItem 
              ...
              isCarouselPaused={!!selectedItem || !!activeFunbox} 
            />
          ))}
        </div>
      </motion.div>
      ...
      <MediaModal item={selectedItem} onClose={handleClose} isMobile={false} onLaunchFunbox={handleLaunchFunbox} />
      <FunboxRuntime session={activeFunbox} onClose={() => setActiveFunbox(null)} />
    </div>
  );
};
